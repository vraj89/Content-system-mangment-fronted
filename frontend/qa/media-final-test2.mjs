...
console.log('Expected 0 ->', hasDashboardButton===0 ? 'PASS' : 'FAIL')
await ctx.close()
...
if(!taskIdForProj){
  console.log('Creating new MEDIA task...')
...
console.log('Task select disabled initially:', await taskSelect.isDisabled()?'YES':'NO')
...
let taskOptions=await taskSelect.evaluate(el=> Array.from(el.options).map(o=>o.value).filter(v=>v))
console.log('Task options for project:', taskOptions.length)
if(taskOptions.length===0){
  console.log('No tasks for this project, trying another')
...
await page.waitForTimeout(1500)
body=await page.textContent('body')
console.log('After file set with both selections, has Upload complete?:', body.includes('Upload complete')?'YES':'NO')
...
let newMedia=null
for(let m of mediaList){ if(m.fileName && String(m.fileName).includes('test-media')){ newMedia=m; break } }
if(!newMedia && mediaList.length) newMedia=mediaList[0]
